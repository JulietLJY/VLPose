...
class LocalWindowPromptSelfAttention(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_heads,
                 window_size,
                 ...
                 with_pad_mask=False,
                 init_cfg=None,
                 prompt_config=dict(deep=deep, num_tokens=5),):
        super().__init__(init_cfg=init_cfg)
        if isinstance(window_size, int):
            window_size = (window_size, window_size)
        self.window_size = window_size
        self.with_pad_mask = with_pad_mask
        self.attn = WindowPromptMSA(
...
class HRFormerPromptBlock(BaseModule):
    expansion = 1
    def __init__(self,
                 in_features,
                 out_features,
                 ...
                 prompt_config=dict(deep=deep, num_tokens=5),
                 **kwargs):
        super(HRFormerPromptBlock, self).__init__(init_cfg=init_cfg)
        self.num_heads = num_heads
        self.window_size = window_size
        self.mlp_ratio = mlp_ratio

        self.norm1 = build_norm_layer(transformer_norm_cfg, in_features)[1]
        self.attn = LocalWindowPromptSelfAttention(
            in_features,
            num_heads=num_heads,
            window_size=window_size,
            init_cfg=None,
...
class HRFomerPromptModule(HRModule):
    def __init__(self,
                 num_branches,
                 block,
                 num_blocks,
